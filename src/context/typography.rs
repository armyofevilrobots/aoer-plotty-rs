...
pub struct Typography {
    ...
    close: bool,
    align: TextAlignment
    // TODO: Add text-align, other stuff.
}
...
impl Typography {
    ...
    pub fn default_font() -> Font {
        let font_data = include_bytes!("../../resources/fonts/ReliefSingleLineOutline-Regular.otf").to_vec();
    ...
    pub fn render(& self, text: &String) -> Result<Geometry<f64>, Box<dyn Error>> {
        ...
        let output_geometries: Vec<Geometry<f64>> = glyphs.iter().map(|g|{
            g.geo.map_coords(|(x,y)|
                (units_per_em *x*Self::mm_per_em(),
                 units_per_em *y*Self::mm_per_em())) //.clone()
        }).collect();
        ...
        let output_geo_collection = match &self.align{
            ...
            TextAlignment::Right => output_geo_collection.translate(-(bounds.max().x-bounds.min().x), 0.0),
            TextAlignment::Center =>output_geo_collection.translate(-(bounds.max().x-bounds.min().x)/2.0, 0.0),
        };
