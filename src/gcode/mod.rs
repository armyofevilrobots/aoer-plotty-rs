...
#[cfg(test)]
mod test {
    use crate::gcode::{AoerPostMachines, PostGeometrySource, post};
    use geo_types::{LineString, MultiLineString, coord};
    use std::iter::zip;

    #[test]
    fn test_post() {
        let post_template = AoerPostMachines::get_machine(AoerPostMachines::BAPv1).unwrap();
        let lines = MultiLineString::new(vec![LineString::new(vec![
            coord! {x: 0.0, y: 0.0},
            coord! {x: 10.0, y: 0.0},
        ])]);
        let program = post(&PostGeometrySource::MultiLineString(lines), &post_template).unwrap();
        let pairs: Vec<(String, String)> = zip(
            program,
            vec![
                "M280 S5",
                "G4 P150",
